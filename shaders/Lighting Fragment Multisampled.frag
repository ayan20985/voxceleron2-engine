...
layout(set = 0, binding = 2) uniform sampler2DMS albedo_sampler;
layout(set = 0, binding = 3) uniform sampler2DMS specular_sampler;
layout(set = 0, binding = 4) uniform sampler2DMS translucent_sampler;
...
float shadow_calculation(vec4 position_shadow_space, float blur)
{
	const vec2 OFFSET = gl_SamplePosition/textureSize(shadow_sampler, 0)*blur;
	const vec2 UV = (position_shadow_space.xy+OFFSET)*.5f+.5f;
...
vec4 textureMS(in sampler2DMS texture, vec2 uv)
{
	...
	return texelFetch(texture, texel_uv, gl_SampleID);
}
...
void main()
{
	...
	const float ALBEDO_ALPHA = textureMS(albedo_sampler, in_uv).a;
	if(ALBEDO_ALPHA < .001f){ out_lighting = vec4(SKY_COLOR, 1); return; }

	//Textures
	const vec3 POSITION = vec4(uniforms.inverse_view*vec4(
		textureMS(position_sampler, in_uv).xyz, 1.f)).xyz;
	const vec3 NORMAL = normalize(mat3(uniforms.transposed_view)*
		(textureMS(normal_sampler, in_uv).xyz*2.f-1.f));
	const vec3 ALBEDO = textureMS(albedo_sampler, in_uv).rgb;
	...
	const vec4 TRANSLUCENT = textureMS(translucent_sampler, in_uv);
	...
	//Shadow
	float shadow = 1.f;
	float signed_normal_dot_halfway = dot(NORMAL, LIGHT);
	if(signed_normal_dot_halfway > 0.f)
	{
		const float SHADOW_FADE_START = 0.f;
		const float SHADOW_FADE_END = 1024.f;
		const float SHADOW_FADE = clamp((SHADOW_FADE_END-CAMERA_DISTANCE)/
			(SHADOW_FADE_END-SHADOW_FADE_START), 0.f, 1.f);

		const vec4 POSITION_SHADOW_SPACE = uniforms.shadow_matrix*vec4(POSITION, 1.f);
		shadow = shadow_calculation(POSITION_SHADOW_SPACE, (1.f-SHADOW_FADE)*60.f);
	}
	...
	//Dot products
	const float NORMAL_DOT_HALFWAY = max(signed_normal_dot_halfway, 0);
