...
{
	//Skip sky
	float position_alpha = texture(position_sampler, fragment_uv).a;
	if(position_alpha == 0){ out_color = 1; return; }
	
	//Vectors
	vec3 position = texture(position_sampler, fragment_uv).xyz;
	vec3 normal = normalize(texture(normal_sampler, fragment_uv).xyz*2.f-1.f);

	//Noise
	vec2 resolution = textureSize(position_sampler, 0);
	...
	for(int i = 0; i < SSAO_KERNEL_SIZE; ++i)
	{
		...
		float sample_depth = texture(position_sampler, offset.xy).z;
